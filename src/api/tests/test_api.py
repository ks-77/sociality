...
from rest_framework.status import (HTTP_200_OK, HTTP_401_UNAUTHORIZED,
                                   HTTP_403_FORBIDDEN, HTTP_405_METHOD_NOT_ALLOWED, HTTP_201_CREATED)
...
class TestApi(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(creator=self.user, media_file=self.media_file, description="Test Post",
                                        location="52.5200,13.4050", creation_date=timezone.now())
        self.story = Story.objects.create(creator=self.user, media_file=self.media_file,
                                          location="52.5200,13.4050", creation_date=timezone.now())
    ...
    def test_create_user(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(reverse('api:customuser_create'),
                                    data={'username': 'testtest', 'password': 'testtesttest'})
        self.assertEqual(response.status_code, HTTP_201_CREATED)

    def test_update_custom_user(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.put(reverse('api:customuser_update', kwargs={'pk': self.user.pk}),
